...
10. **Story 03's `Assign_RejectsClosedTicket` deleted, consolidated into `Assign_ClosedTicket_ThrowsTicketClosed`.** After entry 8 pinned the exact type, the two tests walked the same path and the older one asserted strictly less. Keeping both means two tests that drift apart while one passes for a weaker reason. **No coverage is lost** — every assertion in the deleted test is subsumed by the survivor: `Assert.Throws<TicketClosedException>` is the same exact-type pin, `Assert.Equal` on the full message subsumes the old `Assert.Contains(nameof(TicketStatus.Closed), …)`, and `Assert.IsNotType<InvalidTicketTransitionException>` is redundant once the exact type is pinned to a sealed class that is not it. The survivor additionally asserts `Operation == "assigned"`. Story-03 coverage is consolidated, not weakened.
