...
- **Open question, now answered.** Story 02 registered persistence through the `AddPersistence` extension rather than a domain-declared interface, because with zero aggregates an `IUnitOfWork` would have no callers (constitution §VII). [Story 04](../ticketing-core/04-story-ticket-endpoints-10.md) is that caller and settles the shape: one domain-declared `ITicketRepository`, framework-free, with `SaveChangesAsync` on the repository and **no** separate unit of work. `AddPersistence` stays the only seam the API touches. A second abstraction is earned when a transaction must span two aggregates, and that story must say so.
